# -*- coding: utf-8 -*-
"""Shared: ToM in Large Language Models V3.ipynb
...
The tasks and procedures were updated following the feedback from the reviewers. Inclusion of even stricter controls resulted in a modest decrease in models' performance, when compared with the original version of this study.
...
client = OpenAI(api_key=input("PASTE HERE YOUR OPENAI API KEY HERE: "))
import numpy as np
...
def gpt3(prompt, engine="text-davinci-003", logprobs=1, echo_probs=False, echo_response=False, temperature=0, **kwargs):
  out=client.completions.create(prompt=prompt, engine=engine, logprobs=logprobs, temperature=temperature, **kwargs)
...
def sentence_by_sentence(text, diag_prompt, target_word):
    ...
    out=client.completions.create(engine="text-davinci-003", prompt=diag_prompt, max_tokens=0,temperature=0,logprobs=0, stop=".", echo=True)
    ...
    for i in txt:
        tmp=tmp+i+" "
        out=client.completions.create(engine="text-davinci-003", prompt=tmp+diag_prompt, max_tokens=0,temperature=0,logprobs=0, stop=".", echo=True)
        d=d+[out.choices[0].logprobs.token_logprobs[loc]]
...
txt1 = r"""Here is a bag filled with popcorn. There is no chocolate in the bag. Yet, the label on the bag says "chocolate" and not "popcorn". Sam finds the bag. She has never seen this bag before. Sam doesn't open the bag and doesn't look inside. Sam reads the label. """
### Prompt 1.1: Contents
qcontents = "Sam opens the bag and looks inside. She can clearly see that it is full of"
...
### GPT-3.5 / GPT-4
chat(preamble+txt1+qcontents,temperature=0, model="gpt-4-0613", max_tokens=20)

#### Prompt 1.2: Protagonist's belief
qbelief = "Sam calls a friend to tell them that she has just found a bag full of"
...
### GPT-3.5 / GPT-4
chat(preamble+txt1+qbelief,temperature=0, model="gpt-4-0613", max_tokens=20)
...
### GPT-3.5 / GPT-4
pr11=diag_sentences_chat(preamble+txt1, qcontents, engine="gpt-4-0613",n=3) ### set n to 1000
for i in pr11:
    print(i)
    print(len(pr11[i]))
...
### Reversed task
txtr = r"""Here is a bag filled with chocolate. There is no popcorn in the bag. Yet, the label on the bag says "popcorn" and not "chocolate". Sam finds the bag. She has never seen this bag before. Sam doesn't open the bag and doesn't look inside. Sam reads the label. """
out1r=prob_chat(preamble+txtr+qcontents,temperature=1, max_tokens=3, n=10) ## set n=1000
out2r=prob_chat(preamble+txtr+qbelief  ,temperature=1, max_tokens=3, n=10) ## set n=1000

### True-belief controls
### open container control
qbelief_open = r"""Here is a bag filled with popcorn. There is no chocolate in the bag. Yet, the label on the bag says "chocolate" and not "popcorn". Sam finds the bag. She has never seen this bag before. Sam opens the bag and looks inside. Sam reads the label. """
out2=prob_chat(preamble+txt1+qbelief_open,temperature=1, max_tokens=3, n=10) ## set n=1000

##### Correct label
txt1 = r"""Here is a bag filled with popcorn. There is no chocolate in the bag. The label on the bag says "popcorn" and not "chocolate." Sam finds the bag. She had never seen the bag before. Sam doesn't open the bag and doesn't look inside. Sam reads the label. """
...
txt1 = r"""Here is a bag filled with popcorn. There is no chocolate in the bag. The label on the bag says "popcorn" and not "chocolate." Sam finds the bag. She had never seen the bag before. Sam doesn't open the bag and doesn't look inside. Sam reads the label. A friend calls Sam and tells her that the bag has popcorn in it, and that she should ignore the label that says "chocolate". Sam believes her friend. """out3=prob_chat(preamble+txt1+qcontents,temperature=1, max_tokens=3, n=1000)## set n=1000
...
### Reversed task:
txt1 = r"""In the room, there are John, Mark, a cat, a box, and a basket. John takes the cat and puts it in the box. He closes the box. He leaves the room and goes to school. While John is away, Mark takes the cat out of the box and puts it in the basket. He closes the basket. Mark leaves the room and goes to work. John comes back home and wants to play with the cat. """
...
def process_tsk(tsk, typ="txt", len_out=20, eng="gpt-3.5-turbo", study="S1", reverse=False):
    ...
    if typ in ["txt_open"]:
        ...
        if typ in ["txt_correctlabel"]:
            if tsk["NUMBER"]==1:
                txt =tsk[typ]+" XNAM does not connect to the CX and does not explore its contents. XNAM reads the label on the CX. "
            if tsk["NUMBER"]==17:
                txt =tsk[typ]+" XNAM does not open the box and does not listen to any of the CDs. XNAM reads the label."
            else:
  ...
  out=[]
  for i in [q1,q2]:
      if eng in ["gpt-4", "gpt-3.5-turbo","gpt-4-0314","gpt-4-0613","gpt-3.5-turbo-0301"]:
...
tsks1=[{'txt': 'On the shelf, there is a non-transparent CX. It is full of S1; there is no S2 in it. Yet, the label on this CX says "S2" and not "S1". XNAM walks into the room and notices the CX. XPRO has never seen it before.',
  'o1': 'beer',
  'o2': 'wine',
  'c': 'bottle',
  'xnam': 'Mark',
...
### present protagonist
txt_present=[process_tsk(i, eng=e, typ="txt_present", len_out=n, study="S2",reverse=False) for i in tsks2] ### use reverse=True for reversed tasks